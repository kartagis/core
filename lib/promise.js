...
Bluebird.config({longStackTraces: true, cancellation: true});
 ...
 */
const retry = (fn, {max = 5, backoff = 500} = {}) => Bluebird.resolve().then(() => {
  const rec = counter => Bluebird.try(() => fn(counter).catch(err => {
    if (counter <= max) {
      return Bluebird.delay(backoff * counter).then(() => rec(counter + 1));
    } else {
 ...
 *   }, {max: 3, backoff: 1000});
 */
Bluebird.prototype.retry = retry;

/**
 * Extended Bluebird Promise class with retry functionality
 *
 * @type {typeof Bluebird}
 * @exports LandoPromise
 */
